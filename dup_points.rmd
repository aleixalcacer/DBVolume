...
points_02 <- tibble(x = runif(n, 0, 1), y = runif(n, 0, 1), z = runif(n, 0.2, 1.2))
...
# rotate plotly to set a oblique prespective

scene = list(camera = list(eye = list(x = 1.5, y = 1.5, z = 0.1)))

p <- plot_ly()
p <- p %>% add_trace(type="scatter3d", data=as.tibble(points_01_i), x=~x, y=~y, z=~z, mode="markers", marker=list(size=2, color="green"))
p <- p %>% add_trace(type="scatter3d", data=as.tibble(points_02_i), x=~x, y=~y, z=~z, mode="markers", marker=list(size=2, color="red"))
p <- p %>% add_trace(type="mesh3d", data=as_tibble(points_01_i), x=~x, y=~y, z=~z, opacity=0.1, alphahull=0)
p <- p %>% add_trace(type="mesh3d", data=as_tibble(points_02_i), x=~x, y=~y, z=~z, opacity=0.1, alphahull=0)
p <- p %>% hide_guides()
p <- p %>% layout(scene = scene)

# save plot as png
# orca(p, file = "s_10.png")
...
duplicated_points <- c(FALSE, TRUE)

volumes <- NULL

for(s_i in duplicated_points) {
  print(s_i)
  points_01_i <- points_01_h %>% filter(!in_i)
  points_02_i <- points_02_h %>% filter(!in_i)
  ...
  if (s_i) {
    for (i in seq_len(4)) {
        points_01_i <- rbind(points_01_i, points_01_h %>% filter(in_i))
        points_02_i <- rbind(points_02_i, points_02_h %>% filter(in_i))
    }
  }
  ...
  if (s_i) {
    for (i in seq_len(4)) {
      points_01_i <- bind_rows(points_01_i, points_01_i)
      points_02_i <- bind_rows(points_02_i, points_02_i)
    }
  }
...
```{r}
ggsave("s-volumes.png", p, width = 10, height = 6)
```
